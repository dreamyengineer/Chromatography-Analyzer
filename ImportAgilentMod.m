%% This File is modified specifically to read out .ch Chromatography Data for PSI
% SCALING FACTOR FOR .CH FILE OF TYPE 179 WAS REMOVED BECAUSE IT WAS 
% ERRONEOUS FOR OUR MACHINE

% FOR CASE 179 THE OFFSET VALUE FOR THE START OF DATA IS MODIFIED, VALUE IS
% TAKEN FROM PYHTON LIBRARY RAINBOW CHEMSTATION.PY 'data_start': 0x1800

%% PAY ATTENTION WHEN USING OTHER FILE FORMATS AS THEY MAY HAVE WRONG SCALING

%%
% This file is modified from:
% Copyright (c) 2014 James Dillon (MIT License)
% Original source: https://github.com/chemplexity/chromatography/
%%
% ------------------------------------------------------------------------
...
%
function varargout = ImportAgilentMod(varargin)
% Parse user input
...
switch options.version
    ...
    case {'179'}
        ...
        options.offset.units = 4172; % this is right and corresponds to 'unit':0x104C
        
        fseek(file, 264, 'bof');
        offset =6144;% (fread(file, 1, 'int32', 'b') - 1) * 512; % THIS WAS CHANGED, VALUE TAKEN FROM PYTHON SCRIPT
        ...
        fseek(file, 4724, 'bof');
        intercept =  fread(file, 1, 'float64', 'b');
        data.method.intercept = intercept;

        fseek(file, 4732, 'bof');
        slope = 1; %fread(file, 1, 'float64', 'b'); %this corresponds to the scaling factor at pos 0x127C
        data.method.slope = slope;
        data.method.realSlope = fread(file, 1, 'float64', 'b');
        data.tic = data.tic * slope + intercept;
    ...
    case {'181'}
        ...
        options.offset.inlet = 2492;
        options.offset.units = 4172; % this is right and corresponds to 'unit':0x104C 
        % I ADDED THIS FROM RAINBOW PYTHON 
        %options.offset.scaling_factor = 4732;
        ...
        fseek(file, 4732, 'bof');
        slope = fread(file, 1, 'float64', 'b'); % this is the scaling factor
        data.extraData = slope;
        data.tic = data.tic * slope + intercept;
end
% Close file
fclose(file);
